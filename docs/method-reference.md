...
| `->whereNot($field, $value)`     | Drops rows where `$field` loosely equals `$value`. With `$field` alone, keeps rows where it's empty or missing                                                   |
...
| `->filter($callback)`            | Keeps elements where `$callback` returns true (callback receives plain PHP values); with no callback, removes empty values (`""`, 0, NULL, false); keys are kept |
| `->sort($flags)`                 | Sorts a flat list by value, renumbering keys; `$flags` are PHP sort flags (default `SORT_REGULAR`)                                                               |
| `->sortBy($field, $flags)`       | Sorts rows ascending by `$field`; pass `SORT_NATURAL` to sort numbers the way people read them                                                                   |
| `->unique()`                     | Removes duplicate values from a flat list, keeping the first of each (compares as text, so 1 and `'1'` match)                                                    |
...
| Method                            | Description                                                                                           |
|-----------------------------------|-------------------------------------------------------------------------------------------------------|
| `->column($columnKey, $indexKey)` | Returns one field from every row, like `array_column()`; `$indexKey` keys the values by another field |
...
| `->map($callback)`                | Rebuilds each element from what `$callback` returns (callback receives plain PHP values)              |

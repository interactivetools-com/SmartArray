...
5. [Transforming and Grouping](transforming-and-grouping.md) - Reshaping results with `column()`, `indexBy()`, `groupBy()`, `map()`, and `implode()`.
...
- [Performance](performance.md) - What SmartArray costs vs plain arrays: about 0.002 ms per 25-row page and 300 bytes per row.
